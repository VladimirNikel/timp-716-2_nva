...
int removed(list *lst) {
    	if (*lst != NULL) {
        	list del = *lst;
	        if (((*lst)->prev != NULL) && ((*lst)->next != NULL)) {		//обычный случай
	        	(*lst)->prev->next = (*lst)->next;
			(*lst)->next->prev = (*lst)->prev;
        	}
        if (((*lst)->prev == NULL) && ((*lst)->next != NULL)) {			//голова
        	(*lst)->next->prev = NULL;
            	(*lst) = (*lst)->next;
        }
        if (((*lst)->prev != NULL) && ((*lst)->next == NULL)) {			//хвост
            	(*lst)->prev->next = NULL;
        }
        free(del);
    }
